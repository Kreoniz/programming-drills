This drill focuses on pointer receivers, nil checks, and explicit mutation of shared state. The point is to practice the smallest useful version of the idea, then let the tests force precision. In production code, this skill usually appears inside larger features, where a vague understanding is not enough: edge cases, naming, and failure behavior become part of the API.
